...
pub enum DhtMessage {
    ...
    FindNodeResponse { sender: NodeId, nodes: Vec<NodeInfo> },
    GetPeers { sender: NodeId, info_hash: [u8; 32] },
    GetPeersResponse { sender: NodeId, peers: Vec<PeerEntry>, nodes: Vec<NodeInfo> },
    AnnouncePeer { sender: NodeId, info_hash: [u8; 32], peer_port: u16, score: f64 },
}
...
impl DhtMessage {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        match self {
            ...
            Self::GetPeersResponse { sender, peers, nodes } => {
                buf.push(0x05);
                buf.extend_from_slice(sender);
            ...
            Self::AnnouncePeer { sender, info_hash, peer_port, score } => {
                buf.push(0x06);
                buf.extend_from_slice(sender);
                buf.extend_from_slice(info_hash);
...
mod tests {
    ...
    fn roundtrip_find_node() {
        let msg = DhtMessage::FindNode { sender: test_id(1), target: test_id(42) };
        let decoded = DhtMessage::decode(&msg.encode()).unwrap();
    ...
    fn roundtrip_find_node_response() {
        ...
        let msg = DhtMessage::FindNodeResponse { sender: test_id(1), nodes };
        let decoded = DhtMessage::decode(&msg.encode()).unwrap();
    ...
    fn roundtrip_get_peers_response() {
        let peers = vec![PeerEntry { peer_id: test_id(5), addr: "127.0.0.1:7000".into(), score: 0.85 }];
        let msg = DhtMessage::GetPeersResponse { sender: test_id(1), peers, nodes: vec![] };
        let decoded = DhtMessage::decode(&msg.encode()).unwrap();
    ...
    fn roundtrip_announce_peer() {
        let msg = DhtMessage::AnnouncePeer { sender: test_id(1), info_hash: [0xAB; 32], peer_port: 6881, score: 0.72 };
        let decoded = DhtMessage::decode(&msg.encode()).unwrap();
